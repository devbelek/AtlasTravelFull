...
const TourDetails: React.FC = () => {
  ...
  const [tourDetails, setTourDetails] = useState<TourDetailsInterface | null>(
    null
  );
  const [similarToursArr, setSimilarToursArr] = useState<any[]>([]);
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const toursResults = await axiosGetToursDetails(toursApi, tourId);

        setTourDetails(toursResults);

        const cityResult = await axiosGetCity(toursResults.to_city);
        setCityInfo(cityResult);

        const similarTours = await axiosGetToursSimilar(toursApi, tourId);
        setSimilarToursArr(similarTours);
      } catch (error) {
  ...
  return (
    <main className={styles.tour_detail_page}>
        ...
        <section className={styles.main_info_section}>
          ...
          <div className={styles.main_info}>
            <span className={styles.average_score}>
              {tourDetails?.average_rating}
            </span>
            <span className={styles.reviews_quantity}>
              {getPluralValues(
                tourDetails?.rating_count ?? 0,
                locale === "/ru"
            ...
            <h1 className={styles.title}>
              {translate(
                tourDetails?.title_ru ?? "",
                tourDetails?.title_ky ?? "",
                tourDetails?.title_en ?? ""
              )}
            </h1>
            ...
            <div className={styles.icon_and_text}>
              ...
              <div className={styles.icon_and_text_inner}>
                ...
                <p>
                  {formatDate(tourDetails?.start_tour ?? "")} -{" "}
                  {formatDate(tourDetails?.end_tour ?? "")}
                </p>
              </div>
            </div>
            <p
              ...
              dangerouslySetInnerHTML={{
                __html: translate(
                  tourDetails?.description_ru ?? "",
                  tourDetails?.description_ky ?? "",
                  tourDetails?.description_en ?? ""
                ),
              }}
            ></p>
            <div className={styles.tag_block}>
              {tourDetails?.tags.map((tag) => (
      ...
      <section className={styles.comments_section}>
        <Container isVisible={true}>
          ...
          <div className={styles.comment_block}>
            {tourDetails &&
            tourDetails.comments &&
            tourDetails?.comments.length > 0 ? (
      ...
      <section className={styles.similar_tours}>
        <Container>
          <OffersWithMainCard
            offerTitle={tours("sameTours")}
            slides={similarToursArr}
            content={toursApi}
          />
        </Container>
      </section>
